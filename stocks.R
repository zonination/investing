...
# sp500<-read.csv("https://raw.githubusercontent.com/datasets/s-and-p-500/master/data/data.csv", stringsAsFactors=FALSE)
sp500<-read.csv("stocks.csv", stringsAsFactors=FALSE)

# Doing stuff with dates:
sp500$Date<-as.Date(sp500$Date,"%Y-%m-%d") # Reformatting the dates to make it readable by the system.
# sp500<-subset(sp500,sp500$Date >= as.Date("1926-01-01","%Y-%m-%d")) # S&P 500 was started in 1923; prior history is from Shiller

#We're going to apply a common visual theme to our graphs:
z_theme <- function() {
...
# Master Loop
stocks<-data.frame(0,0,0)
names(stocks)<-c("year","real","percent")
for(f in 1:50){
...
#     stocks<-rbind(stocks,c(f,sp500$future.r[n],                   # Record all history
...
# ggplot(stocks,aes(year,percent))+
#   geom_jitter(alpha=.05,color="limegreen")+
#   ggtitle("Returns After Investing\n(EXPERIMENTAL DATA ONLY)")+
#   ylab("Relative Change after Inflation and Dividends")+
#   xlab("Years Invested in US Stocks")+
#   scale_y_log10(limits=c(.01,100000))+
#   z_theme()